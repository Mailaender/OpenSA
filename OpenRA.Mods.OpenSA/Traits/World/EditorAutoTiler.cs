...
namespace OpenRA.Mods.OpenSA.Traits
{
	...
	public class EditorAutoTiler : IWorldLoaded
	{
		...
		void PaintCell(CPos cell, ushort[] content, int index)
		{
			for (var f = 0; f < 4; f++)
			{
				var tile = new TerrainTile(content[index], (byte)f);
				var subcell = new CPos(cell.X + (f % 2), cell.Y + (f / 2));
				map.Tiles[subcell] = tile;
			}
		}
					...
					if (index != -1)
					{
						if (MatchingBorderCells.TryGetValue(clear, out var validNeighbors))
						{
							if (validNeighbors.All(n => map.GetTerrainInfo(cell + n).Type == autoConnect.Type))
								PaintCell(cell, autoConnect.BorderTransitions, index);
						}
					}
				}
			}
			...
			foreach (var cell in map.AllCells)
			{
				...
				foreach (var autoConnect in customTerrainTemplate.AutoConnect)
				{
					...
					if (index != -1)
					{
						...
						{
							if (validNeighbors.All(n => map.GetTerrainInfo(cell + n).Type == autoConnect.Type))
								PaintCell(cell, autoConnect.Corners, index);
						}
					}
				}
			}
